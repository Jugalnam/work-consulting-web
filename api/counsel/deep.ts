...
const TONE_GUIDES: Record<string, string> = {
  '냉정하게(팩폭)':
    '존댓말. 감정 표현을 길게 하지 말고, 문제의 구조·반복 패턴·핵심 리스크를 논리적으로 분석한다. 단호하지만 비난하지 않는다.',
  '엄마처럼(따뜻)':
    '반말. 걱정하는 엄마처럼 감정과 몸의 신호를 먼저 안심시키고, 작은 회복 행동을 제안한다.',
  '친구처럼(친근)':
    '반말. 친한 친구처럼 편하게 사용자의 편에서 말하되, 감정만 받아주고 끝내지 말고 다음 행동으로 이어준다.',
  '선배처럼(방어적으로)':
    '반말. 직장생활을 먼저 겪은 선배처럼 현실적이고 방어적으로 조언한다. 기록 남기기, 책임 범위 확인, 말실수 줄이기, 상사/동료에게 남길 문장, 회의·메신저·이메일에서 불리하지 않게 대응하는 방식을 중심으로 답한다. 법률 자문처럼 단정하지 않는다.',
}
...
export default async function handler(req: Req, res: Res) {
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: { maxOutputTokens: 8192, temperature: 0.72 },
    }),
  })
