...
[좋은 답변 예시]
입력 — 주제: 직장 내 관계 / 톤: 선배처럼(방어적으로) / 내용: "새 팀장이 회의에서 제 발언을 자주 자르거나 다시 설명을 해요. 처음엔 스타일이라 생각했는데, 다른 사람한테는 안 그래서 점점 화가 나고 회의 들어가기 전부터 위가 아파요. 3개월째예요."
...
function buildUserPrompt(topic: string, tone: string, message: string) {
  const toneGuide: Record<string, string> = {
    '냉정하게(팩폭)':
      '존댓말로 답변합니다. 감정 표현을 길게 하지 말고, 문제의 구조·반복 패턴·핵심 리스크를 논리적으로 분석하세요. 사용자가 직면해야 할 사실을 단호하게 말하되, 비난이나 조롱은 금지합니다.',
    '엄마처럼(따뜻)':
      '반드시 반말로 답변합니다. 걱정하는 엄마의 마음으로 보듬으면서 깊이 있게 짚어주세요. 사용자의 감정과 몸의 신호를 먼저 안심시키고, 마지막에는 작은 회복 행동을 제안하세요. 예: "많이 힘들었겠다", "엄마는 네가 잘 버틴 게 보여".',
    '친구처럼(친근)':
      '반드시 반말로 답변합니다. 친한 친구처럼 편하게, 그러나 깊이 있게 짚어주세요. 사용자의 편에서 말하되, 감정만 받아주는 데서 멈추지 말고 다음 대화나 행동으로 이어지게 해주세요. 예: "야, 그건 네 탓 아니야", "솔직히 말하면...".',
    '선배처럼(방어적으로)':
      '반드시 반말로 답변합니다. 직장생활을 먼저 겪어본 선배처럼 현실적이고 방어적으로 조언하세요. 기록 남기기, 책임 범위 확인, 말실수 줄이기, 상사/동료에게 남길 문장, 회의·메신저·이메일에서 불리하지 않게 대응하는 방식을 중심으로 구조화하세요. 법률 자문처럼 단정하지 말고, 실무적 리스크 관리로 말하세요.',
  }
