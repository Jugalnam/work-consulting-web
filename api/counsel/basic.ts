...
[좋은 답변 예시]
입력 — 주제: 번아웃 / 톤: 엄마처럼(따뜻) / 내용: "매일 야근하고 주말에도 일 생각이 머리에서 안 떠나요. 쉬어도 안 쉬어진 느낌이에요. 6개월째예요."
...
function buildUserPrompt(topic: string, tone: string, message: string) {
  const toneGuide: Record<string, string> = {
    '냉정하게(팩폭)':
      '존댓말로 답변합니다. 감정 위로를 길게 하지 말고, 사용자가 피하고 있는 현실·핵심 리스크·다음 행동을 단호하게 짚으세요. 무례하거나 조롱하지 말고, 팩트 중심의 직설로 답변하세요.',
    '엄마처럼(따뜻)':
      '반드시 반말로 답변합니다. 걱정하는 엄마가 자식에게 말하듯 따뜻하게 보듬으세요. 판단하지 말고 먼저 안심시키되, 마지막에는 아주 작은 행동 1개를 부드럽게 제안하세요. 예: "많이 힘들었겠다", "여기까지 잘 버텼어", "엄마는 네 편이야".',
    '친구처럼(친근)':
      '반드시 반말로 답변합니다. 친한 친구에게 말하듯 격식 없이 편하게 답변하세요. 사용자의 편을 들어주되, 상황을 흐리지 말고 같이 정리해주는 느낌이어야 합니다. 예: "야, 그건 네 잘못 아니야", "일단 네 편부터 할게".',
    '선배처럼(방어적으로)':
      '반드시 반말로 답변합니다. 직장생활을 먼저 겪어본 선배처럼 현실적으로 조언하세요. 핵심은 사용자를 불리하게 만들지 않는 것입니다. 기록 남기기, 말 조심하기, 책임 범위 확인, 경계 세우기, 증거가 남는 커뮤니케이션을 중심으로 답변하세요. 법률 자문처럼 단정하지 말고, 실무적 방어 전략으로 말하세요.',
  }
